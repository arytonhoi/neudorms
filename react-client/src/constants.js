export const API_URL = "https://nudorms-server.herokuapp.com/api";
// export const API_URL = "http://localhost:8070/api";
export const REGISTER_URL = "https://nudorms-server.herokuapp.com";

// LOCAL URL: http://localhost:8070/api
// DEPLOY URL: https://nudorms-server.herokuapp.com/api

// building constants
export const FIND_ALL_BUILDINGS = 'CREATE_ALL_BUILDINGS';
export const FIND_BUILDING_BY_ID = "FIND_BUILDING_BY_ID";
export const CREATE_BUILDING = 'CREATE_BUILDING';
export const DELETE_BUILDING = 'DELETE_BUILDING';
export const UPDATE_BUILDING = 'UPDATE_BUILDING';
export const EDIT_BUILDING = 'EDIT_BUILDING';
export const FILTER_BUILDINGS = 'FILTER_BUILDING';
export const HIGHEST_RATED = 'HIGHEST_RATED';
export const CLEAR_BUILDINGS = 'CLEAR_BUILDINGS';

// picture constants
export const FIND_ALL_PICTURES = 'FIND_ALL_PICTURES';
export const CREATE_PICTURE = 'CREATE_PICTURE';
export const UPDATE_PICTURE = 'UPDATE_PICTURE';
export const DELETE_PICTURE = 'DELETE_PICTURE';
export const FIND_PICTURES_FOR_BUILDING = 'FIND_PICTURES_FOR_BUILDING';

// review constants
export const FIND_ALL_REVIEWS = 'FIND_ALL_REVIEWS';
export const FIND_REVIEW_BY_ID = 'FIND_REVIEW_BY_ID';
export const FIND_REVIEWS_BY_USER = 'FIND_REVIEWS_BY_USER';
export const CREATE_REVIEW = 'CREATE_REVIEW';
export const UPDATE_REVIEW = 'UPDATE_USER';
export const DELETE_REVIEW = 'DELETE_REVIEW';
export const FIND_REVIEWS_FOR_BUILDING = 'FIND_REVIEWS_FOR_BUILDING';
export const FIND_RECENT_REVIEWS = 'FIND_RECENT_REVIEWS';
export const FIND_RECENT_USER_REVIEWS = 'FIND_RECENT_USER_REVIEWS';
export const FILTER_REVIEWS = 'FILTER_REVIEWS';
export const CLEAR_REVIEWS = 'CLEAR_REVIEWS';
export const DELETE_REVIEWS_FOR_BUILDING = 'DELETE_REVIEWS_FOR_BUILDING';

// user constants
export const FIND_ALL_USERS = 'FIND_ALL_USERS';
export const FIND_USER_BY_USERNAME = 'FIND_USER_BY_USERNAME';
export const FIND_BOOKMARKS_FOR_USER = 'FIND_BOOKMARKS_FOR_USER';
export const ADD_USER_BOOKMARK = 'ADD_USER_BOOKMARK';
export const REMOVE_USER_BOOKMARK = 'REMOVE_USER_BOOKMARK';
export const CREATE_USER = 'CREATE_USER';
export const UPDATE_USER = 'UPDATE_USER';
export const DELETE_USER = 'DELETE_USER';
export const PROFILE = 'PROFILE';
export const LOGIN = 'LOGIN';
export const LOGOUT = 'LOGOUT';
export const CLEAR_BOOKMARKS = 'CLEAR_BOOKMARKS';
